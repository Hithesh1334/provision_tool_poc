...
def user_fun(roles_list):
    ...
    def render_rows():
        for index, row in enumerate(st.session_state["rows"]):
            cols = st.columns(4)
            with cols[0]:
                st.markdown(f'<p id="label_tag">User Name</p>', unsafe_allow_html=True)
            ...
            with cols[1]:
                st.markdown(f'<p id="label_tag">Password</p>', unsafe_allow_html=True)
            ...
            with cols[2]:
                st.markdown(f'<p id="label_tag">System Defined Roles</p>', unsafe_allow_html=True)
    ...
    st.markdown(f'<p id="label_tag">Assign roles to user</p>', unsafe_allow_html=True)
