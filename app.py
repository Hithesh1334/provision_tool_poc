...
if 'env_list' not in st.session_state:
    st.session_state['domain_name'] = []
if 'warehosue_info' not in st.session_state:
    st.session_state['warehouse_info'] = []
# resource monitor varibales
if 'rm' not in st.session_state:
    st.session_state['rm'] = ""
if 'rm_monitor_type' not in st.session_state:
    st.session_state['rm_monitor_type'] = ""
if 'rm_frequency' not in st.session_state:
    st.session_state['rm_frequency'] = ""
if 'rm_notify' not in st.session_state:
    st.session_state['rm_notify'] = ""
if 'rm_notify_suspend' not in st.session_state:
    st.session_state['rm_notify_suspend'] = ""
if 'rm_notify_only' not in st.session_state:
    st.session_state['rm_notify_only'] = ""
if 'rm_creditQuota' not in st.session_state:
    st.session_state['rm_creditQuota'] = ""
if 'user_info' not in st.session_state:
    st.session_state['user_info'] = collections.defaultdict(list)
if 'role_name' not in st.session_state:
    st.session_state['role_name'] = ""
if 'role_list' not in st.session_state:
    st.session_state['role_list'] = {"Roles":[]}
if 'role_assign_user_info' not in st.session_state:
    st.session_state['role_assign_user_info'] = collections.defaultdict(list)
...
if "third_page" not in st.session_state:
    st.session_state["third_page"] = False
if "final_page" not in st.session_state:
    st.session_state["final_page"] = False
...
def main():
    ...
    mybar = st.progress(30,text="")
    if st.session_state["second_page"]:
        mybar.progress(70,text="")
    if st.session_state["third_page"]:
        mybar.progress(100,text="")
    # if st.session_state["final_page"]:
    #     mybar.progress(100,text="")
    if st.session_state["first_page"]:
        st.title("Provision Tool")

        with st.container(border=True,key="first_block"):
            st.session_state['project_name'] = st.text_input(label="Please specify your project name",placeholder="Project Name... ",key="project_text_input")
            ...
            st.session_state['domain_name'] = st.text_input(label = "Please specify your business domain",placeholder="Marketing",key = "domains_input")
            st.session_state['domain_name'] = (st.session_state['domain_name'].replace(" ","")).upper()
            if st.session_state['domain_name'] and st.session_state['project_name']:
            ...
            st.session_state['domain_name'] = st.pills(label="Please select the environments required",selection_mode="multi",options=["PROD","DEV","QA","NONPROD","SANDBOX"],disabled= st.session_state['envs'],default=None)

            st.divider()
            if st.session_state["project_setup_spinner"]:
                my_bar = st.progress(0, text="")
            if st.session_state['project_name'] and st.session_state["project_setup_spinner"]:
                my_bar.progress(40,text="")
            if st.session_state['domain_name'] and st.session_state["project_setup_spinner"]:
                my_bar.progress(75,text="")
            if st.session_state['project_name'] and st.session_state['domain_name'] and st.session_state['domain_name']:
        ...
        with st.status(label="Specify the warehouse to create.",expanded=st.session_state['status'][0],state='complete' if st.session_state['state'][0] else 'error') as warehouse_container:
            def render_rows():
                for index, row in enumerate(st.session_state["warehouse"]):
                    cols = st.columns(5)
                    with cols[0]:
                        if st.session_state['domain_name']:
                            row["warehouse_name"] = st.text_input(
                                label=f"Warehouse Name",
                                value=(f"{st.session_state['domain_name']}_Adhoc_wh").upper(),
                                placeholder=(f"{st.session_state['domain_name']}_Adhoc_wh").upper(),
                    ...
                    st.session_state['warehouse_info'].append([row["warehouse_name"],row["warehouse_size"],row["warehouse_type"],row["initially_suspended"]])
            ...
            # rm_creditQuota = ""
            if rm_required: 
                st.session_state['rm'] = st.text_input(label = "Resource Monitor Name",placeholder=" ",value="LOAD_MONITOR",key="resource_monitor")
                st.session_state['rm_monitor_type'] = st.pills(label = "Monitor Type",options=['Account','Warehouse'],key="monitor_type")
                st.session_state['rm_creditQuota'] = st.text_input(label= "CreditQuota",placeholder=" ",key="creditQuota",help="Example: creaditQuota = 10")
                if st.session_state['rm_monitor_type'] == 'Warehouse':
                    st.write("write here warehouse multiselector code")
                st.session_state['rm_frequency'] = st.pills(label="What should be the frequency of resource monitor",options=['Daily','Weekly','Monthly','Yearly'],selection_mode='single')
                st.write("Select the below optinos of how would you like to be notifyed")
                st.session_state['rm_notify'] = st.checkbox(label = "Notify at 70%")
                st.session_state['rm_notify_suspend'] = st.checkbox(label = "Notify and suspend at 85%")
                st.session_state['rm_notify_only'] = st.checkbox(label = "Notify at 95%")

            st.divider()
        
            if st.session_state['warehouse_info'][0][0] and st.session_state["warehouse_spinner"]:
                with st.spinner("In Progress..."):
                    time.sleep(5)
                if rm_required:
                    if st.session_state['rm'] and st.session_state['rm_creditQuota']:
                        # with st.spinner("In Progress..."):
                        #     time.sleep(5)
        ...
        with st.status(label="Specify the users to create.",expanded=st.session_state['status'][1],state='complete' if st.session_state['state'][2] else 'error') as user_block:
                    ...
                    with cols[3]:
                        ...
                        with cl[1]:
                            if st.button("Del", key=f"delete_{index}"):
                                delete("rows",index)
                                st.rerun()  # Force rerun to update the UI
                    st.session_state['user_info'][row['user_name']].append(row["user_name"])
                    st.session_state['user_info'][row['user_name']].append(row["password"])
                    st.session_state['user_info'][row['user_name']].append(row["Roles"])
            ...
            user_keys = list(st.session_state['user_info'].keys())
            user_values = list(st.session_state['user_info'].values())
        ...
        with st.status(label="Specify the roles to create.",expanded=st.session_state['status'][2],state='complete' if st.session_state['state'][2] else 'error') as roles_container:
            st.session_state['role_name'] = st.text_input("Role Name",placeholder=" ",key = "roles")
            st.session_state['role_name'] = st.session_state['role_name'].replace(" ","")
            ...
            if rw_ro:   
                for env in st.session_state['domain_name']:
                        print("in line 373")
                        for value in rw_ro_list:
                            if st.session_state['role_name']:
                                roles = st.session_state['role_name'] + '_' + env + '_' + value 
                            else:
                                roles =   '_' + env + '_' + value 
                            st.session_state['role_list']["Roles"].append(roles)
                    
                df = pd.DataFrame(st.session_state['role_list'])
                st.data_editor(df,num_rows="dynamic",use_container_width=True)
            # else:
            #     # roles = domain_name.upper() 
            #     st.session_state['role_name'] = (st.session_state['role_name'].replace(" ","")).upper()
            #     st.session_state['role_list']["Roles"].append(st.session_state['role_name'])
            st.divider()
            ...
            def render_rows():
                for index, row in enumerate(st.session_state["role_assign_user"]):
                    cols = st.columns(3)
                    with cols[0]:
                        row["Select_user"] = st.multiselect(label ="",options=[key for key,value in st.session_state['user_info'].items()],default=None,key=f"role_assign_user_select_user_{index}",placeholder="Select the users")
                    with cols[1]:
                        row["Select_role"] = st.multiselect(label ="",options=[st.session_state['role_list']['Roles'][i] for i in range(len(st.session_state['role_list']["Roles"]))],default=None,key=f"role_assign_user_select_role_{index}",placeholder="Select the roles")
                    with cols[2]:
                        cl = st.columns(2)
                        with cl[0]:
            ...
            render_rows()
            st.session_state['role_assign_user_info'] = role_assign_user
            print(role_assign_user,"line330")
            if st.session_state['role_name'] and st.session_state["roles_spinner"]:
                with st.spinner("In progress..."):
                    time.sleep(5)
                if rw_ro:
        ...
        if st.button("next",key="first_page_next_button"):
    ...
    if st.session_state["second_page"]:
        with st.status(label="Specify the schemas to create.",expanded=st.session_state['status'][3],state='complete' if st.session_state['state'][3] else 'error') as schema_container:
            ...
            def render_rows():
                for index, row in enumerate(st.session_state["schemas"]):
                    ...
                    with cols[1]:
                        row["Database_name"] = st.multiselect(label ="Database",options=[st.session_state['domain_name'] + "_"+st.session_state['domain_name'][i] for i in range(len(st.session_state['domain_name']))],default=None,key=f"schema_database_{index}",placeholder="")
                    with cols[2]:
                        cl = st.columns(2)
                        with cl[0]:
        ...
        if st.button("next",disabled=st.session_state['save_button']):
            # change_css()
            ...
            # print("line 298",len(role_assign_user),st.session_state['user_info'],st.session_state['user_info'].values(),len(st.session_state['user_info']),list(st.session_state['user_info'].keys()),role_assign_user )
            snowflake_config = {
                "Snowflake": {
                    "ProjectName": st.session_state['project_name'],
                    "user": [
                        {
                            "user_name": key,
                            "password": st.session_state['user_info'][key][1],
                            "default_roles": st.session_state['user_info'][key][2],
                            "roles_to_assign": ["hh"] if len(list(st.session_state['user_info'].items())) == 1 and len(st.session_state['role_assign_user_info']) == 0  else st.session_state['user_info'][key][2] + st.session_state['role_assign_user_info'][key][0]
                        } for key,items in st.session_state['user_info'].items()
                    ],
                    "warehouse": [
                        {
                            "warehouse_name":w[0],
                        ...
                        } for w in st.session_state['warehouse_info']
                    ],
                    "resource_monitor": {
                        "resource_monitor_name":"" if not st.session_state['rm'] else st.session_state['rm'],
                        "frequency":"" if not st.session_state['rm_frequency'] else st.session_state['rm_frequency'],
                        "monitor_level":"" if not st.session_state['rm_monitor_type'] else st.session_state['rm_monitor_type'],
                        "trigger": {
                            "notify" :"" if not st.session_state['rm_notify'] else st.session_state['rm_notify'],
                            "notify_suspend":"" if not st.session_state['rm_notify_suspend'] else st.session_state['rm_notify_suspend'],
                            "notify_only":"" if not st.session_state['rm_notify_only'] else st.session_state['rm_notify_only']
                        }
                    },
                    "env": [st.session_state['domain_name'] + "_" +env for env in st.session_state['domain_name']],
                    "Domains": st.session_state['domain_name'],
                    "roles": st.session_state['role_list']["Roles"],
                    "assign_privileges_to_role": [
                        {
                            "object_name": (
                                st.session_state['domain_name'] + "_" +"PROD" if "_PROD_" in st.session_state['role_list']["Roles"][i] else
                                st.session_state['domain_name'] + "_" +"DEV" if "_DEV_" in st.session_state['role_list']["Roles"][i] else
                                st.session_state['domain_name'] + "_" +"QA" if "_QA_" in st.session_state['role_list']["Roles"][i] else
                                st.session_state['domain_name'] + "_" + "NONPROD" if "_NONPROD_" in st.session_state['role_list']["Roles"][i] else
                                st.session_state['domain_name'] + "_" +"SANDBOX"
                            ),
                            "object_type": "Database",
                            "roles": st.session_state['role_list']["Roles"][i] ,
                            "privilege": (
                                "USAGE" if "_RO" in st.session_state['role_list']["Roles"][i] else
                                ["ALL PRIVILEGES"] 
                                ) 
                        } for i in range(len(st.session_state['role_list']["Roles"]))
                    ],
                    "assign_role_to_user":[
                        {
                            "roles": st.session_state['role_assign_user_info'][key][0],
                            "to_user": key
                        } for key,value in st.session_state['role_assign_user_info'].items() 
                    ],
                    "resource_monitor":[
                        {
                        "rm_name": st.session_state['rm'],
                        "rm_frequency": st.session_state['rm_frequency'],
                        "rm_type": st.session_state['rm_monitor_type'],
                        "rm_notify": st.session_state['rm_notify'],
                        "rm_notify_suspend": st.session_state['rm_notify_suspend'],
                        "rm_notify_only": st.session_state['rm_notify_only'],
                        "creditQuota": "" if not st.session_state['rm_creditQuota'] else int(st.session_state['rm_creditQuota'])
                    }
                    ],
                    "schema":[
                        {
            ...
            st.rerun()
        if st.button("prev",key="second_page_prev_key"):
            st.session_state["first_page"] = True
            st.session_state["second_page"] = False
            st.rerun()
    if st.session_state["third_page"]:
        ...
        if st.button("prev",key="third_page_prev_button"):
            st.session_state["third_page"] = False
            st.session_state["second_page"] = True
            st.rerun()
